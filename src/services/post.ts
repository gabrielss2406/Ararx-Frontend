...
export class PostService {
  ...
  public async getPosts(
    ...
    try {
      const response = await api.get(
        `/posts?page_num=${page_num}&page_size=${page_size}`,
      );
      return response.data.sort((a: PostType, b: PostType) =>
  ...
  public async getPostsFromUser(
    ...
    try {
      ...
      const response = await api.get(
        `/posts/author/${user_handler}?page_num=${page_num}&page_size=${page_size}`,
      );
      return response.data;
    } catch (error) {
      console.error(error);
